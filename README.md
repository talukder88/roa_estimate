...
The tool, first, uses [Newton-Raphson](https://www.math.ubc.ca/~anstee/math104/newtonmethod.pdf) method to compute an equilibrium. If the specified dynamic field has multiple equilibria, one can specify the equilibrium (or a close initial guess) of interest. In a nutshell, the search for the inner-estimate of a maximal RoA around the equilibrium is posed as an optimization problem involving a number of set emptiness constraints (thus nonconvex), which are equivalently posed as SoS constraints via [Positivstellensatz](https://www.mit.edu/~parrilo/ecc03_course/06_positivstellensatz.pdf) theorem. To achieve computational tractability, only a convex sufficient condition is verified employing semidefinite programming (SDP), which serves as a certificate of set emptiness. The maxima is attained through iterative local search.  
